...
import { Send, CheckCircle, AlertCircle, Loader2, GitBranch, Globe, Mail } from "lucide-react";
          ...
          <AnimatedSection delay={0.1}>
            <form onSubmit={handleSubmit} className="space-y-4">
              <div className="grid gap-4 sm:grid-cols-2">
                <div>
                  <label htmlFor="name" className="mb-1.5 block text-xs font-medium text-[var(--color-text-muted)]" style={textFont}>
                    {tr.contact.name}
                  </label>
                  <input
                    id="name"
              ...
              <div>
                <label htmlFor="subject" className="mb-1.5 block text-xs font-medium text-[var(--color-text-muted)]" style={textFont}>
                  {tr.contact.subject}
                </label>
                <input
                  id="subject"
              ...
              <div>
                <label htmlFor="message" className="mb-1.5 block text-xs font-medium text-[var(--color-text-muted)]" style={textFont}>
                  {tr.contact.message}
                </label>
                <textarea
                  id="message"
              ...
              <button
                ...
                className="inline-flex w-full items-center justify-center gap-2 rounded-xl bg-[var(--color-accent)] px-6 py-3 text-sm font-semibold text-[var(--color-bg)] shadow-[var(--shadow-md)] transition-all duration-200 hover:opacity-90 disabled:opacity-60 disabled:cursor-not-allowed sm:w-auto"
                style={textFont}
              >
                {status === "loading" ? (
                  <>
              ...
              {status === "success" && (
                <div className="flex items-center gap-2 rounded-xl border border-[var(--color-success)]/20 bg-[var(--color-success)]/5 p-3 text-sm text-[var(--color-success)]" style={textFont}>
                  <CheckCircle size={16} />
                  {tr.contact.success}
                </div>
              )}
              {status === "error" && (
                <div className="flex items-center gap-2 rounded-xl border border-[var(--color-error)]/20 bg-[var(--color-error)]/5 p-3 text-sm text-[var(--color-error)]" style={textFont}>
                  <AlertCircle size={16} />
                  {tr.contact.error}
                </div>
              )}
          ...
          <AnimatedSection delay={0.2}>
            <div className="space-y-6">
              <div className="rounded-2xl border border-[var(--color-border)] bg-[var(--color-bg)] p-6">
                <h3 className="text-sm font-bold text-[var(--color-text-primary)]" style={textFont}>
